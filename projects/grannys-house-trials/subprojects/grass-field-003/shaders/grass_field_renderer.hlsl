...
// The standard renderer reads CPU-composed cells. The GPU-resident variant
// reads terrain and water separately from compute-owned buffers.
#ifdef GPU_RESIDENT_FLUID
StructuredBuffer<int> gpu_terrain_inches : register(t0);
StructuredBuffer<float> gpu_water_inches : register(t1);

ColumnData LoadColumnData(uint index)
{
    ColumnData result;
    result.water_depth_feet = gpu_water_inches[index] / 12.0;
    result.surface_height_feet =
        ((float)gpu_terrain_inches[index] + gpu_water_inches[index]) / 12.0;
    return result;
}
#else
...
float3 EstimateSurfaceNormal(int cell_x, int cell_z, float3 hit_pos, float col_top)
{
    ...
    if (abs(hit_pos.y - col_top) <= top_epsilon)
    {
        ...
        const float left_h  = LoadColumnData(left_x  + cell_z  * field_width).surface_height_feet;
        const float right_h = LoadColumnData(right_x + cell_z  * field_width).surface_height_feet;
        const float back_h  = LoadColumnData(cell_x  + back_z  * field_width).surface_height_feet;
        const float front_h = LoadColumnData(cell_x  + front_z * field_width).surface_height_feet;
...
TraceResult TraceField(CameraRay ray)
{
    ...
    for (int step = 0; step < max_steps; ++step)
    {
        ...
        const ColumnData cell_data = LoadColumnData(cell_x + cell_z * field_width);
...
float4 PSMain(VSOutput input) : SV_TARGET
{
    ...
    // Fluid experiments upload water_depth_feet so wet cells can be tinted blue.
    const bool water_cell = result.water_depth > 0.01;
    const float3 base_color = water_cell
        ? float3(0.05, 0.34, 0.82)
        : float3(0.24, 0.55, 0.18);
