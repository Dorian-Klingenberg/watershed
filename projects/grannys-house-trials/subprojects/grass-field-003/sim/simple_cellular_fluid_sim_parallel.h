...
// SimpleCellularFluidSimParallel -- deterministic CPU parallel experiment.
//
// Source-cell flow proposals are independent because every proposal reads the
// prior water state. They are computed in parallel into exclusive work slots,
// then replayed serially in the baseline's row-major/neighbor order so floating
// point accumulation order and simulation behavior remain comparable.
...
class SimpleCellularFluidSimParallel final : public IFieldSim
{
public:
    [[nodiscard]] const char* name() const noexcept override
    {
        return "Cellular Water Flow (CPU Parallel Experiment)";
    }
    ...
    void reset(int new_width, int new_depth, std::vector<int> heights_inches) override
    {
        if (new_width <= 0 || new_depth <= 0)
            throw std::invalid_argument("SimpleCellularFluidSimParallel: width and depth must be positive.");

        if (static_cast<int>(heights_inches.size()) != new_width * new_depth)
            throw std::invalid_argument("SimpleCellularFluidSimParallel: seed heights size does not match width * depth.");
    ...
    [[nodiscard]] bool step_once() override
    {
        ...
        std::for_each(
            ...
            [this](CellWork& work)
            {
                compute_source_flow(work);
            });

        // The gather above may run in any order, but this replay exactly
        // matches baseline delta accumulation order.
        for (const CellWork& work : cell_work_)
        {
            if (work.receiver_count == 0)
                continue;

            deltas_[work.source_index] -= work.total_outflow;
            for (int i = 0; i < work.receiver_count; ++i)
            {
                deltas_[work.receiver_indices[static_cast<std::size_t>(i)]] +=
                    work.receiver_inflows[static_cast<std::size_t>(i)];
            }
        }
    ...
    [[nodiscard]] bool render_ui() override
    {
        ...
        ImGui::Text("Hardware threads: %u", std::thread::hardware_concurrency());
        ImGui::TextDisabled("Parallel gather + ordered serial delta replay.");
...
private:
    ...
    struct CellWork
    {
        ...
        std::array<std::size_t, 4> receiver_indices{};
        std::array<float, 4> receiver_inflows{};
    ...
    [[nodiscard]] std::size_t index_of(int x, int z) const
    {
        if (x < 0 || x >= width_ || z < 0 || z >= depth_)
            throw std::out_of_range("SimpleCellularFluidSimParallel: coordinates out of range.");

        return static_cast<std::size_t>(z * width_ + x);
    }

    void build_cell_work()
    {
        cell_work_.clear();
        cell_work_.reserve(water_depths_.size());
        ...
        for (int z = 0; z < depth_; ++z)
        {
            ...
            {
                CellWork work{};
                work.source_index = static_cast<std::size_t>(z * width_ + x);

                // Preserve baseline neighbor order: left, right, up, down.
                if (x > 0)
                    work.neighbor_indices[static_cast<std::size_t>(work.neighbor_count++)] =
                        work.source_index - 1;
                if (x + 1 < width_)
                    work.neighbor_indices[static_cast<std::size_t>(work.neighbor_count++)] =
                        work.source_index + 1;
                if (z > 0)
                    work.neighbor_indices[static_cast<std::size_t>(work.neighbor_count++)] =
                        work.source_index - static_cast<std::size_t>(width_);
                if (z + 1 < depth_)
                    work.neighbor_indices[static_cast<std::size_t>(work.neighbor_count++)] =
                        work.source_index + static_cast<std::size_t>(width_);

                cell_work_.push_back(work);
            }
        }
    }

    void compute_source_flow(CellWork& work)
    {
        work.receiver_count = 0;
        work.total_outflow = 0.0f;

        const float available_water = water_depths_[work.source_index];
        if (available_water <= minimum_water_inches_)
            return;

        const float current_surface =
            static_cast<float>(terrain_heights_[work.source_index]) + available_water;
        std::array<LowerNeighbor, 4> lower_neighbors{};
        int lower_neighbor_count = 0;
        float surface_sum = current_surface;

        for (int i = 0; i < work.neighbor_count; ++i)
        {
            const std::size_t neighbor_i =
                work.neighbor_indices[static_cast<std::size_t>(i)];
        ...
        work.receiver_count = lower_neighbor_count;
        work.total_outflow = total_flow;
        for (int i = 0; i < lower_neighbor_count; ++i)
        {
            const LowerNeighbor& neighbor = lower_neighbors[static_cast<std::size_t>(i)];
            work.receiver_indices[static_cast<std::size_t>(i)] = neighbor.index;
            work.receiver_inflows[static_cast<std::size_t>(i)] =
                neighbor.desired_inflow * flow_scale;
        }
    }
    ...
    void drain_boundary()
    {
        for (int x = 0; x < width_; ++x)
        {
            water_depths_[static_cast<std::size_t>(x)] = 0.0f;
            water_depths_[static_cast<std::size_t>((depth_ - 1) * width_ + x)] = 0.0f;
        }

        for (int z = 0; z < depth_; ++z)
        {
            water_depths_[static_cast<std::size_t>(z * width_)] = 0.0f;
            water_depths_[static_cast<std::size_t>(z * width_ + width_ - 1)] = 0.0f;
        }
    }
    ...
    std::vector<CellWork> cell_work_;
