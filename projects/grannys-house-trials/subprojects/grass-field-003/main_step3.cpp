// grass-field-003 — Step 3: ImGui Integration
...
constexpr wchar_t k_window_title[]      = L"Grass Field 003 — Step 2";
...
struct Application
{
    ...
    {
        ImGui::Begin("Step 3");
        ImGui::Text("ImGui is now running inside the D3D12 frame.");
        ImGui::Text("Frame index: %u", frame_index);
        ImGui::Text("Back buffer size: %u x %u", width, height);
        ImGui::End();
    ...
    void run()
    {
        ...
        while (running)
        {
            ...
            end_frame();
            // Step 3: ImGui frame goes between begin_frame() and end_frame().
