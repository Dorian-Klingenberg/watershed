// grass-field-003 — Step 7: ImGui Controls, Reset, and Mouse Picking
...
constexpr wchar_t k_window_title[]      = L"Grass Field 003 — Step 7";
...
struct Application
{
    ...
    void update_mouse_picking()
    {
        ...
        // ── 1. Reconstruct the same inv_vp the shader is using this frame ────
        const float aspect = static_cast<float>(width) / static_cast<float>(height);
        const XMVECTOR eye    = XMVectorSet(50.f, 80.f, -30.f, 1.f);
        const XMVECTOR target = XMVectorSet(50.f,  0.f,  50.f, 1.f);
        ...
        const XMMATRIX inv_vp = XMMatrixInverse(nullptr, XMMatrixMultiply(view, proj));
        ...
        // ── 3. Unproject the far-clip point into world space ──────────────────
        XMVECTOR far_clip = XMVectorSet(ndc_x, ndc_y, 1.f, 1.f);
        XMVECTOR far_world = XMVector4Transform(far_clip, inv_vp);
        ...
        // ── 4. Ray direction in world space ───────────────────────────────────
        XMFLOAT3 eye_f3;
        XMStoreFloat3(&eye_f3, eye);
        XMFLOAT3 far_f3;
        XMStoreFloat3(&far_f3, far_world);

        const float dir_x = far_f3.x - eye_f3.x;
        const float dir_y = far_f3.y - eye_f3.y;
        const float dir_z = far_f3.z - eye_f3.z;

        // ── 5. Intersect with Y = 0 (the ground plane) ───────────────────────
        ...
        const float t = -eye_f3.y / dir_y;
        if (t < 0.f) return;

        const float hit_x = eye_f3.x + t * dir_x;
        const float hit_z = eye_f3.z + t * dir_z;

        // ── 6. World-space XZ → column index ─────────────────────────────────
    ...
    void initialize_pipeline()
    {
        ...
        // No depth buffer in Step 5 — the full-screen pass needs none.
    ...
    //
    // Camera: fixed overhead-angle view of the 100×100-foot field.
    // Step 8 will replace this with an interactive orbit camera.
    //
    ...
    void update_scene_constants()
    {
        ...
        // Camera positioned above and behind the field (feet), looking at centre.
        const XMVECTOR eye    = XMVectorSet(50.f, 80.f, -30.f, 1.f);
        const XMVECTOR target = XMVectorSet(50.f,  0.f,  50.f, 1.f);
        const XMVECTOR up     = XMVectorSet( 0.f,  1.f,   0.f, 0.f);
    ...
    void run()
    {
        ...
        while (running)
        {
            ...
            // Resolve which column the cursor is over before building ImGui
            // frames, so the "Selected Column" panel has fresh data.
            ...
            // Write updated sim state to the GPU upload buffers.
