...
public:
    ...
    void record_draw(const RendererFrameArgs& args) override
    {
        ...
        pass.coarse_cell_size_cells = args.split_coarse_width > 0
            ? std::max(1u, (args.field_width + args.split_coarse_width - 1u) /
                args.split_coarse_width)
            : 1u;
        pass.water_alpha = 1.0f;
        pass.display_mode = static_cast<std::uint32_t>(
            std::clamp(m_display_mode, 0, 3));
        pass.debug_gain = std::clamp(m_debug_gain, 0.05f, 80.0f);
    ...
    void render_ui() override
    {
        ...
        ImGui::Combo("Water display", &m_display_mode, modes, IM_ARRAYSIZE(modes));
        ImGui::SliderFloat("Debug gain", &m_debug_gain, 0.05f, 40.0f, "%.2f");
        ImGui::TextDisabled("Water is opaque; shallow water is lighter blue.");
    }
