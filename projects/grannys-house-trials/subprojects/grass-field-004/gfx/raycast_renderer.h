...
//   Param 1 → descriptor table: 1 SRV at t0  (column heights, VISIBILITY_PIXEL)
//             The VS ignores the height SRV; only the PS samples it.
//
// Shaders (compiled by CMake via fxc):
//   grass_field_vs.cso  (compiled from grass_field_renderer.hlsl, VSMain)
//   grass_field_ps.cso  (compiled from grass_field_renderer.hlsl, PSMain)
...
private:
    ...
    //   1 — descriptor table: one SRV at t0 (field heights), pixel-visible
    void build_root_signature(ID3D12Device* device)
    {
        ...
        // Param 1: descriptor table — one SRV slot.
    ...
    void build_pso(ID3D12Device* device, DXGI_FORMAT rtv_format)
    {
        ...
        const auto vs  = load_shader_blob(to_utf8(dir + L"shaders/grass_field_vs.cso"));
        const auto ps  = load_shader_blob(to_utf8(dir + L"shaders/grass_field_ps.cso"));
