...
using namespace sea_fed_packet_pump_003;
...
DashboardOptions parse_options(int argc, char **argv)
{
    ...
    for (int index = 1; index < argc; ++index)
    {
        ...
        {
            std::cout << "Usage: sea_fed_packet_pump_003_experiment [--ticks N] [--sleep-ms N] [--no-clear] [--non-interactive]\n";
            std::exit(0);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "Sea-fed packet pump experiment failed: "
                  << exception.what() << '\n';
        return 1;
    }
}
